...
int main()
{
	...
	for(beta=0.1;beta<=2;beta+=temp){
		...
		for (int i = 0; i<selection_list.size(); i++) {
			...
			std::string fn = "selection_mc/aC3/subsample_l" +std::to_string(linear_size)+"Beta"+(Beta)+"h"+(h)+"_"+ std::to_string(i)+ + ".h5";
		...
		std::string fn = "selection_mc/aC3/statics_l" +std::to_string(linear_size)+"Beta"+Beta+"h"+h+ ".h5";
